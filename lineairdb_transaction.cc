...
std::vector<std::string> LineairDBTransaction::get_matching_primary_keys_from_prefix(
    std::string index_name, std::string prefix)
{
  ...
  auto scan_result = tx->ScanSecondaryIndex(
      ...
      prefix,
      std::nullopt,
      [&result, &first_key_checked, &prefix_miss, &prefix, this](
    ...
    const std::string &exclusive_end_key)
{
  ...
  auto scan_result = tx->ScanReverse(
      start_key, end_opt,
      [&result, &exclusive_end_key](auto key, auto)
      {
...
std::optional<std::string> LineairDBTransaction::fetch_last_primary_key_in_secondary_range(
...
std::vector<std::pair<std::string, std::string>> LineairDBTransaction::get_matching_keys_and_values_from_prefix(
    std::string prefix)
{
  if (table_is_not_chosen())
    return {};
